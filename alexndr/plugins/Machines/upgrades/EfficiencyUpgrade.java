 ...
 */
public class EfficiencyUpgrade extends SimpleItem{
	private ConfigItem entry;
	private int tier = 1;
	private float modifier = 0.0F;

	/**
	 * Creates a new Efficiency Upgrade item.
	 * @param plugin The plugin the item belongs to
	 * @param category The category of the item
	 * @param tier The tier of the upgrade
	 */
	public EfficiencyUpgrade(Plugin plugin, Item category, int tier) {
	 ...
	 */
	public EfficiencyUpgrade(Plugin plugin, Item category, int tier, float modifier) {
		super(plugin, category);
		this.tier = tier;
	...
	@Override
	public EfficiencyUpgrade setConfigEntry(ConfigItem entry) {
		this.entry = entry;
	...
	public EfficiencyUpgrade addTooltip() {
		TooltipHelper.addTooltipToItem(this, EnumChatFormatting.RED + "+" + (int)(this.modifier * 100) + "% " + StatCollector.translateToLocal("machines.efficiencyUpgrade.info"));
		return this;
	}
